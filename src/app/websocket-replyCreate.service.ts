...
export class WebsocketReplyCreateService {
  ...
  private sendConnectPacket(postId: string): void {
    if (this.socket && this.socket.readyState === WebSocket.OPEN) {
      ...
      // Construire et envoyer la souscription spécifique
      const subscription = `newReplyCreated.${postId}.prod`;
      this.subscribeToSpecificTopic(subscription);
    }
  }

  private subscribeToSpecificTopic(subject: string): void {
    if (this.socket && this.socket.readyState === WebSocket.OPEN) {
      const subMessage = `SUB ${subject} 1\r\n`;
